package eu.wohlben.qits.stt.telemetry;
 ...
 *
 * <p><b>The behavioural proof is not here and deliberately is not.</b> qits-events owns it:
 * {@code OtelLogBridgeTest} decodes a real {@code ExportLogsServiceRequest} off an offline OTLP stub
 * and asserts identity, severity, body, exception fields and trace correlation, and
 * {@code PackagedLogBridgeIT} repeats the claim against the packaged artifact where the handler's
 ...
 *
 * <p>Note the test asserts config values only, and does so on the running application's own
 * {@code Config}: the shipped {@code src/main/resources/application.properties} is merged into the
 * test configuration, so these are the values a deployment gets. The suite runs with
 * {@code quarkus.otel.sdk.disabled=true} under {@code %test} — nothing is exported during a build,
 * which is exactly why the configuration needs a reader of its own.
 */
@QuarkusTest
